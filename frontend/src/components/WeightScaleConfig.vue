...
<template>
    ...
    <el-dialog v-model="dialogFormVisible" title="权重系数配置" width="600" :close-on-click-modal="false" :append-to-body="true">
...
function convertWeightConfig(data: WeightConfigData): WeightConfigData {
  ...
  for (const [category, config] of Object.entries(data)) {
    ...
    for (const [key, value] of Object.entries(config)) {
      // 保留原始键名，仅转换值
      result[typedCategory]![key] = typeof value === 'string'
