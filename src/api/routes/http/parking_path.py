...
def _snap_end_pose_to_lane(
    ...
    if nearest is None or nearest[3] >= SNAP_LANE_DISTANCE_THRESHOLD:
        return None
    proj_x, proj_y, lane_heading, dist, lane_id = nearest
    ...
    if requested_heading is not None:
        opposite_heading = normalize_angle(lane_heading + math.pi)
        direct_diff = abs(normalize_angle(requested_heading - lane_heading))
        opposite_diff = abs(normalize_angle(requested_heading - opposite_heading))
        if opposite_diff < direct_diff:
            lane_heading = opposite_heading
    return proj_x, proj_y, lane_heading, dist, lane_id
